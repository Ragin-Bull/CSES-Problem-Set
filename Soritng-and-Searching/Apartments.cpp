...
void solve(){
    ...
    int lower_ptr = 0, upper_ptr=0;
    while(lower_ptr<m and upper_ptr<n){
        int apt = apartments[lower_ptr];
        if(desired[upper_ptr]<=apt+k and desired[upper_ptr]>=apt-k){
            acquired++;
            upper_ptr++;
            lower_ptr++;
        }
        else if(desired[upper_ptr]>apt+k){
            lower_ptr++;
        }
        else if(desired[upper_ptr]<apt-k){
            upper_ptr++;
